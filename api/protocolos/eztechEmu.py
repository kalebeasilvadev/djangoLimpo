...
class protocoloEztechEmu():
        ...
        try:
            ...
            if len(status) == tamanho:                
                for i, c in enumerate(status):
                                    ...
                                    if bicos_b in self.bicos:
                                        if bicos_b == bico_andamento:
                                            ...
                                            if bicos_b in self.encerrantes:
                                                print(f'bico {bicos_b} frentista {frentista} 1')
                                                self.encerrantes.update({bicos_b:{'encerrante':encerrante,'frentista':frentista}})
                                            else:
                                                print(f'bico {bicos_b} frentista {frentista} 2')
                                                self.encerrantes.update({bicos_b:{'encerrante':encerrante,'frentista':frentista}})
                                            self.bicos_abastecendo[bicos_b] = {'bico':bicos_b}
                    ...
                    elif c == "L" and self.estado_rel[i] == "G":
                        self.estado_rel[i] = "L"
                        bico = self.montaHexLado(i)
                        print(self.bicos_abastecendo)
                        bicos_remove = []
                        for bicos in self.bicos_abastecendo:
                            bicos = bicos
                            if bicos in bico :
                                ...
                                if qtd > 0.01:
                                    bico_banco = Bicos.objects.filter(bico_emulado=bicos)
                                    ...
                                    if ultimo_enc == 0.00:
                                        ...
                                        if dif > 0.1:
                                            ...
                                            abastecimento = Abast.objects.create(bico=bico_banco[0].bico,cartao="Nao Identificado",produto=produto,valor=preco_padrao,enc_ini=ultimo_enc,enc_fim=enc_ant,quantidade=dif,total=total_padrao,tipo = self.tipo_default) 
                                    ...
                                    total = qtd * preco
                                    abastecimento = Abast.objects.create(bico=bico_banco[0].bico,cartao=frentista,produto=produto,valor=preco,enc_ini=enc_ant,enc_fim=encerrante,quantidade=qtd,total=total,tipo = tipo_chave)
                                    bico_banco.update(encerrante=encerrante)
                                    ...
                                    print(f'bico {bico_banco[0].bico}')
                                    print(f'bicos_abastecendo {self.bicos_abastecendo}')
                                    bicos_remove.append(bico_banco[0].bico)
                                else:
                                    if bicos in self.bicos_atualizados:                                
                                        preco = (int(self.bicos_atualizados[bicos]['preco']))/1000
                                        tipo_chave = self.bicos_atualizados[bicos]['tipo']
                                        ...
                                        tipo_chave = self.tipo_default
                                        produto_preco = self.produto(bicos, tipo_chave)
                                        preco = (int(produto_preco))/1000
                                    
                                    print(f'frentista = {frentista}')
                                    self.abastecimentos.update({int(self.encerrante(bicos)):{
                                        "bico":bicos,
                                        "frentista":frentista,
                                        "enc_ini":encerrante,
                                        "enc_fim":0,
                                    ...
                                    print(f'bico {bicos}')
                                    print(f'bicos_abastecendo {self.bicos_abastecendo}')
                                    bicos_remove.append(bicos)
                                    log = loggers("automacao").info(msg='abastecimento zerado',prefix="[Gravando]")
                        for x in bicos_remove:
                            print(f'bicos remove {x}')                            
                            self.bicos_abastecendo.pop(x)
        except Exception as e:
            track = traceback.format_exc()
            print(track)
            print(e)
                                        ...
                                        if bico_produto:
                                            ...
                                            if produto_preco and status_troca:
                                                if status_troca[1] == "L":
                                                    ...
                                                    encerrante = self.encerrante(a)
                                                try:
                                                    log = loggers("automacao").info(msg=str(self.encerrantes[a]),prefix="[Default]")                                              
                                                    frentista = self.encerrantes[a]['frentista']
                                                except:
                                                    frentista = ""
                                                    self.encerrantes.update({a:{'encerrante':encerrante,'frentista':frentista}})
                                    ...
                                    if a in self.bicos:
                                        ...
                                        if bico_produto:
                                            ...
                                            if produto_preco:
                                                ...
                                                resposta = f"""U{a} preco = {produto_preco} trocado? {troca_preco}"""    
                                                encerrante = self.encerrante(a)
                                                self.encerrantes.update({a:{'encerrante':encerrante,'frentista':''}})                                                   
                                                log = loggers("automacao").info(msg=resposta,prefix="[Default]")                   
                                                time.sleep(self.tempo_cmd)
                                            print('Preco padrao setado')
                                            log = loggers("automacao").info(msg='Preco padrao setado',prefix="[Default]")
    ...
    def abasts(self):
        try:
            if len(self.abastecimentos):
                val = 0
                for abast in self.abastecimentos:
                    abasts = self.abastecimentos[abast]
                    print(abasts)
                    enc_ini = abasts['enc_ini']
                    bicos = abasts['bico']
                    tipo_chave = abasts['tipo']
                    preco = abasts['preco']
                    frentista = abasts['frentista']
                    enc_fim = int(self.encerrante(bicos))/100
                    qtd = enc_fim - enc_ini              
                    if qtd > 0.1:
                        bico_banco = Bicos.objects.filter(bico_emulado=bicos)
                        bico_produto = self.bico(bicos)
                        ...
                        log = loggers("automacao").info(msg=f" bicos_atualizados = {self.bicos_atualizados}",prefix="[Gravando]")
                        try:
                            self.bicos_atualizados.pop(bicos)
                        except:
                            ...
                        if preco == 0 :
                            preco = int(self.produto(bico_produto, self.tipo_default))/1000 
                        total = qtd * preco
                        log = loggers("automacao").info(msg=f"bico {bicos} quantidade {qtd}  preço {preco} valor abastecido {total}  frentista {frentista}",prefix="[Gravando]")        
                        print(f"bico={bicos},cartao={frentista},produto={produto},valor={preco},enc_ini={enc_fim},enc_fim={enc_fim},quantidade={qtd},total={total},tipo = {tipo_chave}")
                        abastecimento = Abast.objects.create(bico=bico_banco[0].bico,cartao=frentista,produto=produto,valor=preco,enc_ini=enc_ini,enc_fim=enc_fim,quantidade=qtd,total=total,tipo = tipo_chave)
                        bico_banco.update(encerrante=enc_fim)
                        val = abast
                        break
                if val > 0:
                    self.abastecimentos.pop(val)
        except Exception as e:
            track = traceback.format_exc()
            print(e)
            print('erro ao gravar no banco de dados o abastecimento')
